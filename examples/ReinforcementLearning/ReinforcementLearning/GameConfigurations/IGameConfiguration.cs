...
namespace ReinforcementLearning.GameConfigurations {
    public interface IGameConfiguration {
        ...
        int SkippedFrames {
            get;
        } // used to skip frames during play (reduces memory usage and improves performances but builds the dataset slower)

        float
            StartingEpsilon {
            get;
        } // starting exploration rate. Max value is 1 (exploration), it decays during training up to 0 (exploitation)
    ...
    public interface IImageGameConfiguration : IGameConfiguration {
        ImageStackLayout
            ImageStackLayout {
            get;
        } // defines whether images will be stacker vertically or horizontally, if more than one image is required to form NN input       
        ...
        FramePadding
            FramePadding {
            get;
        } // what padding to apply to every frame, used to focus the NN to a specific area of the image
    }
